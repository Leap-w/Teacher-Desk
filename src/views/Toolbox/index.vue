 ...
 * 由六个 store 用各自既有的 normalize* 重新加载（见 utils/backup.ts 顶部说明）。
 */
    ...
    >
      ...
      <section class="danger-zone">
        ...
        <dl class="zone-item">
          <dt>清空示例数据</dt>
          <dd>
            删除首次打开时自动生成的示例记录（示例学生 / 课程 / 待办 /
            请假），以及这些学生产生的座位约束。你自己新增的记录不受影响；但若你把某条示例记录改成了自己的内容，它同样会被删掉——确认前请先核对名单。座位方案保留，示例学生占用的座位会在下次打开「座位表」时自动释放。
          </dd>
          <dt>清空全部数据</dt>
    ...
    <AppModal v-model="importOpen" title="导入备份（合并）" :width="560">
      <template v-if="importMeta">
        ...
        <p v-if="importHasChanges" class="note-text">
          备份里引用了本机没有的学生时（例如从别的设备导入），相关座位会显示为空、相关座位约束会显示成「已删除学生」。
        </p>
      </template>
      <template #footer>
