 ...
 *
 * 要做的事：把一个「日期 + 上午 / 下午」当**可比较的时间**用——算时长、判先后、
 * 守「登记的是既成事实」这条守卫。日期一律走字符串键与 `Date.UTC` 日序，不做十进制
 * 日期算术（两处若各算各的，跨月 / 跨年会各自出错，§9.11 的教训），所以只留这一份实现。
 *
 * **使用方：请假模块（Phase 5）。** Phase 7B 的周末返家按拍板口径只记「是否返家」，
 * 不消费这里的登记端点件（见 `types/weekend.ts` 与开发手册 §9.17）。
 */
 ...
 *
 * 两条规则（Phase 5 口径；日后复用它记登记端点的模块同款）：
