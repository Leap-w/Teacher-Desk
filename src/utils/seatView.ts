 ...
 *
 * **两个视角各自画成什么样子**（2026-09-15 按需求方反馈纠正排序列，见 `viewRoomItems`）：
 *
 ...
 *   窗**整体转到对面**（讲台在上、前门右上、后门右下、窗到左墙），列号反过来
 *   9→1 从左到右，**排也跟着倒过来**：第 1 排紧挨顶部的讲台、第 7 排在最下。
 *
 * 两个视角是同一间教室的**正反两面**（画面互为 180°），**但排号 / 座位号是物理号**：
 * 第 1 排永远是「紧挨讲台的那排」，63 号永远是末排末尾。所以两条必须同时成立——
 * 排序列要跟着讲台走（否则会出现「讲台顶在第 7 排头上」这种自己打自己的图），
 * 而每排显示的**编号**取自物理坐标（`viewRowUnits(slotRow)` 的 `slotRow` 就是物理排号，
 * 换视角不会让「第三排」换人）。
 *
...
export const VIEW_NOTES: Record<SeatView, string> = {
  ...
  student: '学生视角：讲台在上、第 1 排紧挨讲台，座位号从右到左递增（排号 / 座位号仍是物理号）',
}
 ...
 * 现在排序列随讲台翻，两个视角互为 180°，**紧挨讲台的那一排永远是第 1 排**。
 * 排号并没有跟着换：`row-1` 在两个视角都指紧挨讲台那一排，教师说「第三排」时
 * 两个视角指的仍是同一排人（见模块顶部说明）。
 *
