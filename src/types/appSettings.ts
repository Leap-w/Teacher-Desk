 ...
 * - `semesterEnd`   期末日期 → **学期进度终点**，也是 `countdownTarget` 的默认目标
 * - `heroBackground` Hero 背景 → **首页**
 * - `heroTitle`     Hero 文案（倒计时卡的标题）→ **首页**
 * - `heroSubtitle`  Hero 副标题（问候与日期之下的一行自定义说明，空则不显示）→ **首页**
 * - `countdownTarget` 倒计时目标（**三选一**，见 `COUNTDOWN_TARGETS`）→ **首页 Hero 倒计时卡**
...
/**
 * 倒计时目标：**从已配置的三个日期里选一个**（v3.0.5-rc）。
 *
 * 刻意不做「再填一个任意日期」的第四种：那样三份日期与一个目标日期会各说各话，
 * 「改一处两边同步」立刻失效——需求方 2026-09-15 拍板也是「从已有日期里选一个当目标」。
 * 值与 `AppSettings` 的日期字段**同名**，取日期时直接 `settings[countdownTarget]`。
 */
export type CountdownTargetKey = 'serviceStart' | 'semesterStart' | 'semesterEnd'

/** 倒计时目标选项（第一项为默认；`label` 给设置页，`short` 给 Hero 卡片的角标） */
export const COUNTDOWN_TARGETS: { value: CountdownTargetKey; label: string; short: string }[] = [
  { value: 'semesterEnd', label: '期末日期（距离期末考试）', short: '期末' },
  { value: 'semesterStart', label: '开学日期（距离开学）', short: '开学' },
  { value: 'serviceStart', label: '支教开始日期（距离出发）', short: '出发' },
]
...
export interface AppSettings {
  ...
  serviceStart: string
  /** 首页 Hero 倒计时卡倒数到哪个日期（从上面三个日期里选） */
  countdownTarget: CountdownTargetKey
  /** 打开应用时默认视图（路由路径；`/` = 首页） */
