...
/**
 * 教室视角（V1.1.2 Phase 1 起；**v3.3.0 改为只镜像列，2026-09-15 补正排序列**）：
 * - `teacher`：讲台在下、第 1 排紧挨讲台、列号左→右递增；
 * - `student`：讲台在上、第 1 排仍紧挨讲台、列号右→左递增（同一间教室的正反两面）。
 * **排号 / 座位号 / 座位数据都是物理号，两个视角指着同一排人**；
 * 视角只改变**渲染顺序**，不改变任何座位数据（见 `utils/seatView.ts`）。
 */
