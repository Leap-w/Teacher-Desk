...
export interface Student {
  ...
  gender: Gender
  /**
   * 身份证尾号（选填，建议 4 位）。**用于重名消歧**（v3.3.1）：
   * 同名学生在界面上显示成「旦增卓玛（3287）」，不重名时只显示姓名。
   * v3.4.0 起也进档案页的检索范围（`utils/studentQuery.ts` 的 `searchText()`）——
   * 报名表 / 学籍表上印的就是尾号，教师拿着它能直接把人搜出来。
   *
   * 消歧为什么用身份证尾号而不是学号：学号自 Phase 5A 起是选填列，真实班级里
   * 常常大面积空缺（清一色的「—」区分不了任何人）；而重名恰恰集中在同一家族 /
   * 同一村落的孩子身上，他们的身份证尾号天然不同。
   * 不做「必须 4 位数字」的硬校验——身份证尾号可能是 X，写死数字会把合法值挡在门外；
   * 输入约束交给表单，数据层只做去空白与截断（见 `normalizeStudent`）。
   */
  idCardSuffix?: string
  /**
