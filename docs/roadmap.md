...
> 同步自 `docs/开发手册.md` §10（2026-09-10 Phase 6 `v0.10.0` 交付后、路线优化时更新）。两份文档冲突时以开发手册为准。
>
> **2026-09-10 路线优化（需求方拍板）**：① 下一个交付是**稳定性增量**（备份提醒 + PWA 正式图标 + 小修打包，不占 Phase 编号）；② **跨标签页 / 双入口同步推迟到 Phase 9**，与后端接入一起设计；③ **Phase 7 分两步**：先抽公共流程骨架（7A），再用骨架实现周末管理（7B）；④ **考勤记录继续留在未排期**（「班级概况」的「出勤」依赖它，故 Phase 8 先接人数与值日汇总）。
>
...
## 近期增量 ⏭ 稳定性增量（**下一开发小步**，不占 Phase 编号）

> 2026-09-10 拍板。功能面已覆盖班主任每天的高频动作（看课 / 记事 / 查人 / 排座 / 批假 / 排值日），下一个交付先解决「用起来是否可靠」。**跨标签页 / 双入口同步不在本增量内**——推迟到 Phase 9 与后端接入一起设计，避免同一件事做两遍。

- **备份提醒**：备份目前是**手动**的、没有任何提醒，保护效果全看教师记不记得导出（技术债 #7）。距上次导出超过阈值时在工作台给出提示，一键跳到 `/toolbox` 导出
- **PWA 正式图标**：把当前 SVG 占位（`public/favicon.svg` / `icon.svg` / `icon-maskable.svg`）换成正式 PNG（192 / 512 / maskable / `apple-touch-icon`），装到手机桌面不再是占位图
- **小修打包**（均已记录在案、逐条修复）：数据管理概览「从未打开过的模块显示 0」（#9）、旧课表键不在「清空示例数据」范围内（#10）、「删除确认文案在关闭动画期短暂变空」、`viewport-fit=cover` 缺失、课程抽屉未接线回车保存、`services/index.ts` 注释口径超前（均为 §五 #6）
- **交付判据**：四项验证 + 交付前独立审查 + 运行时自检全过；**既有数据零迁移**（只加提示与图标，不动存储结构）
- **明确不做**：跨标签页同步（→ Phase 9）、自动 / 定时备份（只提醒、不代劳）、UI Kit 字号 token 化（见未排期）

## Phase 7 ⏭ 周末管理（稳定性增量之后）
...
- PWA 完善：离线体验打磨、更新提示（正式 PNG 图标已并入稳定性增量）
