...
## v1.1.1 —— Phase 5A：学生档案升级（2026-09-12）

> **一版只动学生档案**：① **Excel 批量导入**（本阶段主体）；② **宿舍改固定 8 间**；③ **`seatNumber` 撤出界面**；④ **卡片顺序与内容调整**；⑤ 班委可自定义、学号改选填。**不新增模块 / 路由 / 一级导航**，**不碰座位图 / 请假 / 值日 / 周末 / 课表**。排序、搜索增强、学生头像、卡片视觉重构、批量编辑、云端头像存储、自动排座新策略**一行未写**（Phase 5B，交付时明确禁止提前开发）。详细设计与取舍见开发手册 **§9.24**。
